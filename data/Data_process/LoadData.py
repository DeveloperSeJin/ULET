...
def extract_classes(label_info):
    """
    - classes: ['left hand', 'right hand', 'feet', 'tongue'] 같은 클래스 이름 리스트
    - y_minus1: (N,) 형태의 정수 배열, 값은 0부터 시작 (원래 1~K 이면 -1 적용)
    """

    d = label_info['data']  # object 배열(= MATLAB cell)
    flat = np.array(d, dtype=object).ravel()

    y_minus1 = None

    # flat 안의 각 원소는 보통 (1,1) 구조화 배열이며 dtype.names에 ('classes','y',...)가 있음
    for el in flat:
        # el이 구조화 배열인지 확인
        if hasattr(el, 'dtype') and getattr(el.dtype, 'names', None):
            names = el.dtype.names

            # 2) y 추출 (있고 비어있지 않은 경우)
            if ('y' in names) and (y_minus1 is None):
                try:
                    y_cell = el['y'][0, 0]  # 보통 (N,1) 또는 (N,) 숫자 배열
                    if getattr(y_cell, 'size', 0) > 0:
                        y = np.array(y_cell).reshape(-1)
                        # 보통 1부터 시작하므로 -1 적용
                        y_minus1 = (y.astype(np.int64) - 1)
                except Exception:
                    pass

        # 둘 다 구했으면 종료
        if (y_minus1 is not None):
            break
...
class LoadBCIC_E(LoadData):
    ...
    def get_epochs(self, tmin=-4.5, tmax=5.0, bandpass = False,resample = None,baseline=None):
        ...
        self.y_labels = extract_classes(label_info)  # (N,) 형태, 값은 0부터 시작
        # print(self.y_labels)
        self.x_data = epochs.get_data()*1e6
        eeg_data={'x_data': self.x_data,
                  'y_labels': self.y_labels,
...
class LoadBCIC_2b:
    ...
    def get_label(self,label_path):
        ...
        return extract_classes(label_info)
