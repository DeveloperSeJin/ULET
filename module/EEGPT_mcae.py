...
#         mlp_hidden_dim = int(dim * mlp_ratio)
#         self.mlp = MLP(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
#     def forward(self, x, context, freqs=None):
#         y = self.attn(self.norm1(x), context, freqs)
#         if self.return_attention: return y
#         x = x + self.drop_path(y)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=(64, 1000), patch_size=16, patch_stride=None, embed_dim=768):
        super().__init__()
        self.img_size = img_size            # C, T [62, 2560]
        self.patch_size = patch_size        # 32
        self.patch_stride = patch_stride    # 32
        if patch_stride is None:
            self.num_patches = ((img_size[0]), (img_size[1] // patch_size))
            # ((62), (2560 // 32)) = ((62), (80))
            # ((56), ,(1024 // 32)) = ((56), (32))
        else:
            self.num_patches = ((img_size[0]), ((img_size[1] - patch_size) // patch_stride + 1))# C, N (62, 80)
            # (62, (2560 - 32) // 32 + 1) = (62, 80)
            # (56, (1024 - 32) // 32 + 1) = (56, 32)
    ...
    def forward(self, x):
        ...
        x = self.proj(x).transpose(1,3) # B, 80, C(62), D(128)
        return x
class PatchNormEmbed(nn.Module): # 논문 차이 EEGPT_mcae_fineune에선 Norm 미사용
    """ Image to Patch Embedding
    """
    def __init__(self, img_size=(64, 1000), patch_size=16, patch_stride=None, embed_dim=768):
        super().__init__()
    ...
    def forward(self, x):
        ...
        x = self.unfold(x) # 32, 32, 4960
        
        x = x.transpose(-1,-2) # 32, 4960, 32
        
        x = x.view(B, C, -1, self.patch_size).contiguous() # [32, 62, 80, 32]
        x = x.transpose(1,2) # [32, 80, 62, 32]
...
class EEGTransformerReconstructor(nn.Module):
    ...
    def __init__(
        ...
        use_inp_embed = True,
        embed_dim=768, # 128
        reconstructor_embed_dim=384, # 128
        depth=6, # 8
        num_heads=12, # 4
        mlp_ratio=4.0,
        qkv_bias=True,
    ...
    def forward(self, x, chan_ids=None, mask_x=None, mask_y=None):
        ...
        if chan_ids is None:
            C, N        = self.num_patches # 62, 80
            chan_ids = torch.arange(0,C)   # 62
        ...
        # -- get freqs for RoPE
        # 논문 차이 chan_ids, mask_x, mask_y 파라미터를 사용하며 더 세분화된 reconstruction 로직
        ...
        if mask_y is not None:
            mask_y       = mask_y.to(x.device) # [2975]
...
class EEGTransformerPredictor(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_pos_embed: # False
            self.pos_embed   = nn.Parameter(torch.zeros(1, 1, embed_num, predictor_embed_dim)) # 1, 1, 128, 128
            trunc_normal_(self.pos_embed, std=init_std)
        
        self.mask_token      = nn.Parameter(torch.zeros(1, 1, embed_num, predictor_embed_dim)) # 1, 1, 128, 128
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        # --
    ...
    def forward(self, x, mask_x=None, mask_t=None, timesteps = None, context = None):
        # conditions: (Nq, D) as qurey for downstream 
        ...
        if timesteps is not None:
            ...
            emb = timestep_embedding(timesteps.long(), self.embed_dim, repeat_only=False)# B, 128
            emb = self.time_embedding(emb) # B, 80
        else:
            emb = None
        ...
        # 논문 차이 mask_x, mask_t 처리 로직이 구체적이며, 마스킹 기반 예측 구조 반영
        ############## Mask x ###############
        # -- get freqs for RoPE
        ...
        B, N, eN, D    = x.shape # [32, 320, 1, 128]
        x = x.flatten(1,2) # 32, 320, 128
...
class EEGTransformer(nn.Module):
    ...
    def __init__(
        ...
        drop_path_rate=0.0,
        norm_layer=nn.LayerNorm,
        patch_module=PatchEmbed,# PatchNormEmbed / 차이 EEGPT_mcae_fineune에선 Norm 사용
        init_std=0.02,
        interpolate_factor = 2.,
        return_attention_layer=-1,
        **kwargs
        ...
        self.patch_embed = patch_module(
            img_size=img_size,# C, T [62, 2560] 
            patch_size=patch_size, # 32
            patch_stride=patch_stride, # 32
            embed_dim=embed_dim) # 128
        self.num_patches = self.patch_embed.num_patches# ((62), (80))
        # --
        ...
        self.time_embedding = nn.Sequential(
            nn.Linear(self.embed_dim, embed),
            nn.SiLU(),
    ...
    def prepare_chan_ids(self, channels):
        ...
        return torch.tensor(chan_ids).unsqueeze_(0).long() # 1, C
    ...
    def forward(self, x, chan_ids=None, mask_x=None, mask_t=None, timesteps = None):
        # x.shape B, C, T
        # mask_x.shape mN, mC
        # mask_t.shape mN
        ...
        x = self.patch_embed(x) # (62, 80)
        # patch가 80개인 62채널의 각 path의 time point 128
        B, N, C, D = x.shape # 32, 80, 62, 128
        ...
        # -- add channels positional embedding to x
        x = x + self.chan_embed(chan_ids.long()).unsqueeze(0) # (1,C) -> (1,C,D) 1, 62, 128
        # 32, 80, 62, 128
        
        if mask_x is not None:
            mask_x = mask_x.to(x.device) # [38, 12]
            x = apply_mask(mask_x, x)# B, mN, mC, D [32, 32, 12, 128]
            B, N, C, D = x.shape
        ...
        x = x.reshape((B, N, self.embed_num, -1)) # [32, 32, 4, 256]
...
if __name__=="__main__":
    ...
    model = EEGTransformer(
        ...
        init_std=0.02,
        qkv_bias=True, 
        norm_layer=partial(nn.LayerNorm, eps=1e-6)).to('cuda')
    
    x = torch.rand(32,62,2560).to('cuda')

    mask_x, mask_y = make_masks(model.num_patches)
    ...
    # a = model2(out)
    # print(a, a.shape) 
    # a = model2(out, mask_x=mask_x, mask_t=None)
    a = model2(out)
    print(f'perdictor output: {a.shape}')

    model3 = EEGTransformerReconstructor(
        num_patches=model.num_patches,
        patch_size=32,
        embed_dim=128,
        reconstructor_embed_dim=128,
        depth=8,
        embed_num = 4,
        num_heads=4,
        mlp_ratio=4.0,
    ...
    b = model3(a, mask_y = mask_y)
    print(f'reconstructor output: {b.shape}')


    # C, N = model.num_patches # 62, 80
    ...
    # print(f'r {r.shape} vs y {y.shape}')
    # print(f'h {h.shape} vs z {z.shape}')